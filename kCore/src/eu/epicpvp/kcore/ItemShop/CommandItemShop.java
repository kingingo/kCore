		...
		if(args.length==0){
			...
			if(player.hasPermission(PermissionType.ALL_PERMISSION.getPermissionToString())){
				if(args[0].equalsIgnoreCase("addc")){
					if(args.length>=2){
						player.sendMessage(TranslationManager.getText(player, "PREFIX")+shop.addCategory(UtilNumber.toInt(args[1]), args[2], UtilItem.RenameItem(player.getItemInHand(), "§a"+args[2].replaceAll("_", " "))));
					}else{
						player.sendMessage(TranslationManager.getText(player, "PREFIX")+"/shop addc [SLOT] [PageName]");
					}
					...
					if(args.length==2){
						...
						player.sendMessage(TranslationManager.getText(player, "PREFIX")+"/shop delc [CSLOT]");
					}
				}else if(args[0].equalsIgnoreCase("getc")){
					for(int i = 9; i < 35; i++){
						if(shop.getShop().getItem(i)!=null){
							player.sendMessage(TranslationManager.getText(player, "PREFIX")+"ID: "+i+" ITEM:"+shop.getShop().getItem(i).getItemMeta().getDisplayName());
						}
					}
					...
					if(args.length==6){
						...
						player.sendMessage(TranslationManager.getText(player, "PREFIX")+shop.add(UtilNumber.toInt(args[1]), UtilNumber.toInt(args[2]), hand, UtilNumber.toInt(args[3]),UtilNumber.toInt(args[4]),UtilNumber.toInt(args[5])));
						...
						player.sendMessage(TranslationManager.getText(player, "PREFIX")+shop.add(UtilNumber.toInt(args[1]), -1, hand, UtilNumber.toInt(args[2]),UtilNumber.toInt(args[3]),UtilNumber.toInt(args[4])));
					}else{
						player.sendMessage(TranslationManager.getText(player, "PREFIX")+"/shop addi [CSLOT] [SLOT] [BUY] [SELL] [PAGE]");
					}
					...
					if(args.length>=1){
						...
						player.sendMessage(TranslationManager.getText(player, "PREFIX")+"/shop deli [CSLOT] [SLOT] [PAGE]");
					}
