			...
			if(args.length==0){
				...
				if(args[0].equalsIgnoreCase("saveTemplate")){
					if(args.length==2){
						...
						player.sendMessage(TranslationHandler.getText(player, "PREFIX")+"/tc saveTemplate [Name]");
					}
				}else if(args[0].equalsIgnoreCase("createChest")){
					if(args.length==3){
						treasureChest.addTreasureChest(player.getItemInHand(), args[1],  args[2]);
						player.sendMessage(TranslationHandler.getText(player, "PREFIX")+"§aDie Treasure Chest "+args[2]+" mit dem Template "+args[1]+" wurde erstellt.");
					}else{
						player.sendMessage(TranslationHandler.getText(player, "PREFIX")+"/tc createChest [Template] [Chest]");
					}
				}else if(args[0].equalsIgnoreCase("addItem")){
					if(args.length==3){
						treasureChest.getChest(args[1]).addItem(player.getItemInHand(), Integer.valueOf(args[2]));
						player.sendMessage(TranslationHandler.getText(player, "PREFIX")+"§aDas Item wurde zu der Chest hinzugef§gt!");
					}else{
						player.sendMessage(TranslationHandler.getText(player, "PREFIX")+"/tc addItem [Chest] [Nenner]");
					}
					...
					if(args.length==2){
						...
						player.sendMessage(TranslationHandler.getText(player, "PREFIX")+"/tc start [Name]");
					}
				}
			}
		}
