				...
				if(ev.getPlayer().isSneaking()){
						if(ev.getRightClicked() instanceof Player){
							...
							for(PotionEffect e : ((Player)ev.getRightClicked()).getActivePotionEffects()){
								...
								ev.getPlayer().sendMessage(TranslationHandler.getText(ev.getPlayer(), "PREFIX")+"§cDu musst noch §4"+UtilTime.formatMili( (timer.get(ev.getPlayer().getName())-System.currentTimeMillis()) )+"§c warten");
							}
						}
				}
			}
