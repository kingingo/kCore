...
public class CommandItem implements CommandExecutor{
	...
	@me.kingingo.kcore.Command.CommandHandler.Command(command = "item", sender = Sender.EVERYONE)
	public boolean onCommand(CommandSender sender, Command cmd, String arg2,String[] args) {
		if(sender instanceof Player){
			...
			if(player.isOp()){
				if(args.length==0){
					player.sendMessage(Language.getText(player, "PREFIX")+"/item glow");
					player.sendMessage(Language.getText(player, "PREFIX")+"/item add [Line]");
					player.sendMessage(Language.getText(player, "PREFIX")+"/item clear");
					player.sendMessage(Language.getText(player, "PREFIX")+"/item del [Line]");
				}else{
