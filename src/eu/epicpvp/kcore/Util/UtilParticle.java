...
public enum UtilParticle {
	...
	static {
		...
		try {
			...
			nmsNetworkGetVersion = ReflectionUtilities.getMethod(nmsFieldNetworkManager.getType(), "getVersion", new Class[]
			{ ioNettyChannel });
		} catch (Exception e) {
	...
	private void sendToPlayer(Player player, Location location, float offsetX, float offsetY, float offsetZ, float speed, int count, int[] extra) throws Exception {
		...
		if (ReflectionUtilities.getVersion().contains("v1_8"))
			...
			try {
				...
				{ String.class, Float.TYPE, Float.TYPE, Float.TYPE, Float.TYPE, Float.TYPE, Float.TYPE, Float.TYPE, Integer.TYPE }).newInstance(new Object[]
				{ name, Float.valueOf((float) location.getX()), Float.valueOf((float) location.getY()), Float.valueOf((float) location.getZ()), Float.valueOf(offsetX), Float.valueOf(offsetY), Float.valueOf(offsetZ), Float.valueOf(speed), Integer.valueOf(count) });
				...
				ReflectionUtilities.getMethod(connection.getClass(), "sendPacket", new Class[0]).invoke(connection, new Object[]
				{ packet });
			} catch (Exception e) {
				throw e;
			}
	...
	public void display(float speed, int amount, Location center, double range) throws IllegalArgumentException {
		...
		try {
			for (Player player : UtilServer.getPlayers())
				sendToPlayer(player, center, 0, 0, 0, speed, amount);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void display(float offsetX, float offsetY, float offsetZ, float speed, int amount, Location center, double range) throws IllegalArgumentException {
		...
		try {
			for (Player player : UtilServer.getPlayers())
				sendToPlayer(player, center, offsetX, offsetY, offsetZ, speed, amount);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void sendBlockCrack(Player player, Location location, int id, byte data, float offsetX, float offsetY, float offsetZ, float speed, int count) throws Exception {
		...
		sendToPlayer(player, location, offsetX, offsetY, offsetZ, speed, count, new int[]
		{ id, data });
	}
	...
	public void sendBlockDust(Player p, Location location, int id, float offsetX, float offsetY, float offsetZ, float speed, int count) throws Exception {
		...
		sendToPlayer(p, location, offsetX, offsetY, offsetZ, speed, count, new int[]
		{ id });
	}
	...
	protected static int getVersion(Player p) {
		try {
			...
			Object version = ReflectionUtilities.getVersion().contains("1_7") ? nmsNetworkGetVersion.invoke(network, new Object[]
			{ channel }) : Integer.valueOf(47);
	...
	private static boolean isPlayerInRange(Player p, Location center) {
		...
		if (p.getLocation().getWorld().getUID() != center.getWorld().getUID())
			return false;
		if ((distance = center.distanceSquared(p.getLocation())) > 1.7976931348623157E+308D)
			return false;
