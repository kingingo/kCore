...
public class CommandK implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String arg2, String[] args) {
		if (args.length == 0) {
			...
			System.out.println("/k update [Group]");
				...
				if (args.length == 2) {
					...
					if (player != null) {
						...
						if (!pplayer.getGroups().isEmpty()) {
							System.out.println("Rank: " + pplayer.getGroups().get(0).getName());
						}
						for (Permission perm : pplayer.getPermissions()) {
							System.out.println("P: " + perm.getPermission());
						}
						for (Group g : pplayer.getGroups()) {
							System.out.println("G: " + g.getName());
							for (Permission perm : g.getPermissions()) {
								System.out.println("G_P: " + perm.getPermission());
							}
						}
					}
				}
