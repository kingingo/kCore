...
public class InventoryPageBase extends CraftInventoryCustom {
	...
	public boolean useButton(Player player, ActionType type, ItemStack item, int slot) {
		if (!isSlot(slot, "useButton(Player,ActionType,ItemStack,int)"))
			return true;
	...
	public void setItem(int index, org.bukkit.inventory.ItemStack item) {
		if (!isSlot(index, "setItem(int,ItemStack)"))
			return;
	...
	public int getSlotSort() {
		if (getSize() == InventorySize._9.getSize()) {
			if (getItem(4) == null)
				return 4;
			else if (getItem(5) == null)
				return 5;
			else if (getItem(3) == null)
				return 3;
			else if (getItem(6) == null)
				return 6;
			else if (getItem(2) == null)
				return 2;
			else if (getItem(7) == null)
				return 7;
			else if (getItem(1) == null)
				return 1;
			else if (getItem(8) == null)
				return 8;
		...
		if (slots == null) {
			for (int i = 0; i < getSize(); i++) {
				if (getItem(i) == null || getItem(i).getType() == Material.AIR) {
					if (getItem(i) == null)
						setItem(i, new ItemStack(Material.IRON_FENCE));
	...
	public IButtonOneSlot getButtonOneSlot(int slot) {
		if (!isSlot(slot, "getButtonOneSlot(int)"))
			return null;
